...
parser.add_argument('--emb_path', type=str, default='None', help='directory containing word embeddings')
...
parser.add_argument('--encoding', type=str, default='utf-8', help='encoding for embeddings')
...
if args.emb_path != 'None':
    emb_path = args.emb_path
    vect_path = os.path.join(args.data_path.split('/')[0], 'embeddings.pkl')   
    vectors = {}
    with open(emb_path, 'rb') as f:
        for l in f:
            line = l.decode(args.encoding).split()
            word = line[0]
            if word in vocab:
...
emb = basename(normpath(args.emb_path))[:-4]    
    
if args.mode == 'eval':
    ckpt = args.load_from
else:
    ckpt = os.path.join(args.save_path, 
        '{}_{}_train_{}_{}'.format(emb, args.num_topics, args.train_embeddings, args.seed))
...
if args.train_embeddings and embeddings!=None:
    model.rho.weight.data.copy_(torch.nn.parameter.Parameter(embeddings))
...
def visualize(m, show_emb=False):
    ...
    #queries = ['computer', 'sports', 'religion', 'man', 'love', 
     #           'intelligence', 'money', 'politics', 'health', 'people', 'family']
    #queries = ['консалтинговый', 'огромный', 'переводить', 'крах', 'исходить', 'темный'] lenta
    queries = ['новость', 'боевик', 'женщина']
...
if args.mode == 'train':
    ...
    with torch.no_grad():
        ...
        if args.train_embeddings:
            ...
            queries = ['woman', 'computer', 'sports', 'religion', 'man', 'love', 
                            'intelligence', 'money', 'politics', 'health', 'people', 'family']
            print('\n')
